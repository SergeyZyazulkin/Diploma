...
public class GeneralPolygonsBuilderImpl implements GeneralPolygonsBuilder {

    @Override
    public List<GeneralPolygon> buildGeneralPolygons(GeometryFactory geometryFactory, Hull convexHull)
    ...
    private GeneralPolygon findGeneralPolygon(GeometryFactory geometryFactory, Point current, Point previous) {
        ...
        do {
            current.visit();
            points.add(current);
            edges.add(geometryFactory.createEdge(current, previous, false));
            temp = current;
